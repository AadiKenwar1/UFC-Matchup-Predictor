...
PREPROCESSED_PARQUET = DATA_DIR / "ufc_preprocessed.parquet"
FEATURES_PARQUET = DATA_DIR / "ufc_features.parquet"


def _parquet_cache_ready() -> bool:
    return PREPROCESSED_PARQUET.is_file() and FEATURES_PARQUET.is_file()


def _parquet_cache_disabled() -> bool:
    v = os.environ.get("UFC_DISABLE_PARQUET_CACHE", "").strip().lower()
...
def _get_preprocessed_data():
    ...
    if _df_preprocessed is None:
        if _parquet_cache_ready() and not _parquet_cache_disabled():
            _df_preprocessed = pd.read_parquet(PREPROCESSED_PARQUET)
            _ensure_date_column(_df_preprocessed)
        else:
...
def _get_features_data():
    ...
    if _df_features is None:
        if _parquet_cache_ready() and not _parquet_cache_disabled():
            _df_features = pd.read_parquet(FEATURES_PARQUET)
            _ensure_date_column(_df_features)
        else:
